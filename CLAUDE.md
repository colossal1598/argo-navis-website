...
- **No pill shapes.** Buttons, badges, tags, inputs use small-radius rectangles. Never `rounded-full` — exceptions: avatars, toggle knobs.
- Sharp and simple: depth from surface-color steps and borders, not box-shadows; decoration must serve function. Avoid the generic-AI look (stock glassmorphism, purple-gradient clichés, meaningless blobs) — the User rejects pages that look AI-generated.
...
- Voice: direct and human, short sentences, no marketing fluff ("innovative solutions", "seamless experience" — delete). Real numbers beat adjectives; lead with the reader's pain, not the technology. If a sentence could appear unchanged on a competitor's site, rewrite or delete it.
